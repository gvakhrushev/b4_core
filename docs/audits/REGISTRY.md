...
| **A26** | 2026-07-31 | High | Post-pivot structural sizing fixed the stop at `maxStop`/`MinStop` for every entry, so a deep Pro Max short was sized with its liquidation INSIDE the peak the market had already printed (entry near `T` after a 60-70% fall, where 30-60% bounces are routine) and the base-`φ` band was unreachable at any price | Fixed | `StructuralLeverage.shortStructStop` / `longStop` → `clamp(base g-stop, extreme, delta bound)` | StructuralAB.t.sol |
