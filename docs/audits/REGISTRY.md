...
| **A33** | 2026-08-01 | High | Worse than reported: the liquidation invariant has never been evaluated against a real position — its exercise counter is zero over a full campaign, and zero with the pre-change ghost body too, so it has passed vacuously since it was written. `crankA` itself never registers, so the gap is in how the campaign drives the handler. Every ghost-backed invariant in the file is `assertFalse(ghost)` and shares the doubt | Partially fixed | anchors now sampled; ghost reframed onto the product guarantee; the vacuity marked in place, not silenced | Protocol.invariant.t.sol |
