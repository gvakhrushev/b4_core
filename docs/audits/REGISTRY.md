...
| **A33** | 2026-08-01 | High | The liquidation invariant has never been evaluated against a real position — exercise counter zero over a full campaign, and zero with the pre-change ghost too, so it has passed vacuously since it was written. Every ghost-backed invariant in the file is `assertFalse(ghost)` and shares the doubt. One cause found and fixed: `targetContract` fuzzes ALL public functions of the target, and the handler inherits forge-std `Test`, so the campaign spent its budget on hundreds of inherited assertion helpers instead of the ~23 protocol actions | Partially fixed | `targetSelector` restriction; anchors sampled; ghost reframed onto the product guarantee; residual left as a disabled assertion carrying its evidence | Protocol.invariant.t.sol |
