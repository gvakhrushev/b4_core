...
- **`lockPrices`** is all-or-nothing: it commits only if *every* directional asset prices non-zero inside `Calendar.SNAPSHOT_WINDOW` (1 hour); otherwise it reverts so a later call in the window retries. Settlement USDC is fixed at 1 USD. Missing the window makes the interval unreportable — a liveness cost, not a custody one.
