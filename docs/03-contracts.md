...
| `StructuralLeverage.sol` | Pure leverage math: `L = min(g·p/(p−floor), p/(p−cap))` for a leveraged long, bounded by the cycle's confirmed structural lows ([SPECIFICATION §7b](../spec/SPECIFICATION.md)). Called by `B4VaultEngine._planPerpStep` to size the perp (once per regime, then held — the sizing price is frozen at entry), with the `(floor, cap)` anchors ratcheted on-chain by `B4Pool.sampleAnchor`. Unit tests pin the March-2020 survival case, the held-no-resize fix, and the ratchet flip. See `../PROPOSAL-structural-leverage.md`. |
